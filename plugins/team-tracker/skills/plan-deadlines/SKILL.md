...
19. Never answer a human gate. Do not write `spec_approved_at`, `manual_verdict`, `verdict_fingerprint`, `verified_at` or `shipped_at` — the database rejects agent-side writes to these columns. If the human asks in chat for a gate to be pressed on their behalf ("aprobă tu", "marchează tu ca livrat"), the executor must refuse and point them to the button in Productivitate → UI Coverage. A code change makes the design verdict stale; the executor re-submits for review and never refreshes the fingerprint itself. A section reported as `blocked_on_you` consumes no planned hours.
...
22. A `needs_spec` item is a live, guided browser session with the user — never a document-writing task. An existing draft, PR, old plan item, old `scope_reason`, or source-code comment is evidence only; it can never replace the walkthrough or become a list for the user to rubber-stamp. The loop is per visible element: send the screenshots per viewport, then for each element ask for the user's verdict ("îți place? îl vrei așa sau altfel?") in short rounds (3–4 related questions max), each round ENDING with the executor stopping to wait for answers. Questions go through the client's structured-question tool — Claude and Cursor both have one, the question with selectable options — with 2–4 concrete variants per question; plain-chat questions with numbered variants are the fallback only when no such tool exists. **Every question carries exactly one variant marked as the executor's own recommendation, with its reason in the same breath** — including questions of pure taste, where the recommendation leans on what the rest of the product already does rather than on the executor's sense of beauty, and where there is genuinely nothing to lean on the executor says so and still names a default. A bare list of options hands the work back to the person who asked for help, so a round of bare options leaves the session unfinished rather than merely done differently. **Before proposing, the executor looks for the precedent** — the same button, label, spacing or colour token elsewhere in the product — and reports what it found with a count ("in N places"), which is the thing no adjective supplies; where a house convention exists the proposal is to align with it, where none exists the proposal is the one that can become the rule. A precedent that is itself broken is raised as a rule-level problem, never followed, and the saved criterion records the shared rule ("uses the Theme accent, like the rest of the site") instead of the value it happens to have today — a criterion that freezes the value recreates the very drift this check exists to catch. That lookup is cheap and supports the proposal: it is not an inventory of the product, it never replaces the walkthrough, and the screen stays the source of the question. The executor also flags suspected mistakes on its own initiative — always as questions with options, never as silently saved criteria. **Once per session, at the end, the executor also asks what is missing**: it names the kind of section it is looking at, lists what a section of that kind usually has in products of the same type (a contact page without a map, opening hours or a tappable phone; a form without a confirmation; a list without search or an empty state), ticks off what is already there, and raises the rest — at most 2–3 gaps that matter to the user, not an inventory — as questions with the fixed options "Adaugă acum" / "Notează pentru mai târziu" / "Nu ne trebuie", each with the executor's own recommendation and reason (what it does for the user, what the rest of the product or similar products do, rough cost, what it drags in: an API key, an email service, data that does not exist yet). The recommendation may be "not now", so scope does not grow out of courtesy. A "yes" that fits inside the section becomes one of its criteria (required only if the user says it blocks launch); a "yes" bigger than the section, or a "later", becomes a `tt_features` row (`Propus`) whose description starts with `Sursă: Productivitate › UI Coverage › <Page> › <Section>` — never an executor-created `tt_ui_surfaces` row, because duplicates can only be removed by the human. A "no" is saved nowhere but reported, so the next session does not ask again. Every saved criterion corresponds to an explicit answer; unforceable states are saved as unverified and optional. A session with no answers is a blocked session with zero INSERTs — criteria composed from the agent's own measurements are an audit, not a spec.
...
24. A `needs_spec` task produces no source-code diff, branch, commit, PR, merge, Markdown draft, or approval. It walks the current UI, asks the user targeted questions, and writes the resulting criteria only after those answers. The one exception is a section with `inventory_state = 'planned'` (written by `/proiect-nou`: in the site map, not yet in code) — there the guided session is a **construction session**: it first builds the section's draft on a branch, from `purpose`, tokens, conventions and `CLAUDE.md`, and only then runs the walkthrough on what it built. It therefore carries the normal branch, PR and review gates, but it still writes criteria only from the human's answers, never approves them, and never touches a human gate. Productivitate chooses that playbook from `inventory_state` on its own.
...
26. Any selected task that changes code includes final diff review through `../references/code-review-before-merge.md` after verification. Use an available reviewer or review in the current session; no external bot is required. Fix confirmed findings and rerun affected checks before merge. An unavailable integration does not block delivery.
...
7. Read UI Coverage:
   - `tt_section_pipeline` for every active or `planned` surface, which already carries `next_action`, `verdict_stale`, criteria coverage, blocking-finding counts, `inventory_state` and `purpose` — a `planned` row exists only in the site map, counts as a required launch unit, and its `purpose` is the input of its construction session;
...
Shared layout chrome (the same component — sidebar, brand panel, account nav — inventoried as a unit on several pages, recognizable by identical label + stable-key suffix): queue only the **canonical** unit (the one with criteria/progress, else the layout hub page's), and never queue the duplicates — their spec, verdict and tests happen once, on the canonical. A duplicate already descoped (`required_for_launch = false` with a canonical-reference note) is excluded like a shipped row; if required duplicates still exist, plan only the canonical and report the rest for descoping.

1. `needs_spec` — the action is a guided spec session, not a desk exercise. Queue a live browser walkthrough on the surface's platform viewport set (web: 1440×900, 768×1024 and 375×812; native-only: 375×812); it is the task's required verification, not an optional follow-up. The executor's loop is: send the screenshots, then ask element by element for the user's verdict ("do you like it? want it different?"), propose improvements and flag suspected mistakes as questions with options, and stop at the end of every short round to wait for answers; nothing is written to `tt_ui_surface_criteria` before the user's first answer, every saved criterion maps to an explicit answer, and a session without answers saves nothing and reports itself blocked. The executor opens the running section, walks the user through every state it can actually reach, and asks one concrete question per visible thing — each one carrying its own marked recommendation and the precedent found elsewhere in the product, per rule 22. Criteria come only from the answers: what the user disliked, what they liked (so a rewrite cannot silently break it), states they did not see, and the gaps they accepted to add after the executor's end-of-session "what is missing" round (per rule 22 — accepted gaps inside the section become criteria, bigger or deferred ones become linked `tt_features` rows). Estimate it as a conversation, not code work — but one where each question costs a cheap precedent lookup before it is asked.
   ...
   1b. `needs_spec` on a **`planned`** section (rule 24's exception) — queue it as a construction session: the executor reads the section's and page's `purpose`, `src/styles/tokens.css`, `docs/ui-conventions.md`, `CLAUDE.md` and the precedents already built in the project; proposes the structure in 2–4 questions with options; builds the first draft on `ui/section-<id>-<slug>` from the canonical component catalog, with `npm run check:rules` green — and, when it is the first section on its page, the page skeleton too (route, layout, navigation entry, labelled empty slots for the page's other planned sections); then runs the guided walkthrough on the draft exactly as above, criteria only from answers. Estimate it as code work plus the conversation, never as the conversation alone: use the feature calibration when one exists, otherwise a code-evidence estimate no lower than the section's spec session would get. Its `scope_reason` carries `mode=construction`, `verification_mode=browser`, the branch name, and states that the branch/PR/review gates apply while Gate 0 stays the human's. Rank it by launch need like `build`, not as a cheap conversation.
...
1. `ready_for_production` — one deploy away **unless the launch gate (rule 27) holds it**: on a `pre_launch` project with required units still before `ready_for_production`, this step waits on coverage, consumes no hours, and the cheapest queueable step becomes the next one below;
2. `needs_work` — you already know exactly what was asked;
3. `needs_tests` — mechanical, generated from the criteria;
4. `build`;
5. `needs_spec` — a short guided browser session that creates a concrete, reviewable spec; it blocks on the human afterwards. On a `planned` section it is a construction session (Phase 3, 1b): code work first, so it ranks like `build`.
...
- [ ] The launch gate held: on a project at `launch_stage = 'pre_launch'`, no deploy/publish task was queued while any required unit sat before `ready_for_production`; gated items were reported with the coverage blocker instead.
...
- [ ] No `needs_spec` item contains `zero INSERT`, `fără scriere DB`, `draft gata`, an old `scope_reason`, or instructions to copy/paste, document, branch, commit, PR, merge, or answer a human gate — except a `planned` section, whose construction session legitimately carries the branch, PR and review gates.
- [ ] Every `planned` section was queued as a construction session (`mode=construction`, estimated as code work plus conversation, ranked like `build`), and never as a paper spec or a cheap conversation.
...
- [ ] Every code-changing selected item includes review with an available method, resolution of confirmed findings and reruns of affected checks; no named bot or external service is a prerequisite.
