...
- [ ] No `needs_spec` item contains `zero INSERT`, `fără scriere DB`, `draft gata`, an old `scope_reason`, or instructions to copy/paste, document, branch, commit, PR, merge, or answer a human gate — except a `planned` section, whose construction session legitimately carries the branch, PR and Bugbot gates.
