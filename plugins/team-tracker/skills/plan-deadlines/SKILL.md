...
22. A `needs_spec` item is a live, guided browser session with the user — never a document-writing task. An existing draft, PR, old plan item, old `scope_reason`, or source-code comment is evidence only; it can never replace the walkthrough or become a list for the user to rubber-stamp. The loop is per visible element: send the screenshots per viewport, then for each element ask for the user's verdict ("îți place? îl vrei așa sau altfel?") in short rounds (3–4 related questions max, via a structured-question tool when the client has one), each round ENDING with the executor stopping to wait for answers. The executor also proposes improvements and flags suspected mistakes — always as questions with options, never as silently saved criteria. Every saved criterion corresponds to an explicit answer; unforceable states are saved as unverified and optional. A session with no answers is a blocked session with zero INSERTs — criteria composed from the agent's own measurements are an audit, not a spec.
