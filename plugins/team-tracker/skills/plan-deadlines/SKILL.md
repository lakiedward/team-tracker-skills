...
22. A `needs_spec` item is a live, guided browser session with the user — never a document-writing task. An existing draft, PR, old plan item, old `scope_reason`, or source-code comment is evidence only; it can never replace the walkthrough or become a list for the user to rubber-stamp. The session writes no criterion before the user's first answer: show the state (screenshot or preview), ask ONE concrete question, stop and wait. A session with no answers is a blocked session with zero INSERTs — criteria composed from the agent's own measurements are an audit, not a spec.
...
1. `needs_spec` — the action is a guided spec session, not a desk exercise. Queue a live browser walkthrough on the surface's platform viewport set (web: 1440×900, 768×1024 and 375×812; native-only: 375×812); it is the task's required verification, not an optional follow-up. The executor's loop is show → ask one question → stop and wait for the answer; nothing is written to `tt_ui_surface_criteria` before the user's first answer, and a session without answers saves nothing and reports itself blocked. The executor opens the running section, walks the user through every state it can actually reach, and asks one concrete question per visible thing. Criteria come only from the answers: what the user disliked, what they liked (so a rewrite cannot silently break it), and states they did not see. Estimate it as a conversation, not code work.
