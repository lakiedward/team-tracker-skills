...
`low · medium · high · xhigh · max · ultracode`. It mirrors Claude Code's effort levels (tells whoever builds it how
...
- **xhigh** — large/subtle: an L surface with tricky logic, or UI/UX that's both visible and non-trivial (multiple screens/states, responsive).
...
`max`/`ultracode`. Valid values only: `low|medium|high|xhigh|max|ultracode`.
...
#<id> [<priority>/<type>] <title>
  → Recomandare: <Implementează|Amână|Respinge> — <one-line rationale> (mărime: <S|M|L|XL> · efort: <low|medium|high|xhigh|max|ultracode>)
...
```
...
SET
  ...
  description = COALESCE(description, '') || E'\n\n--- Evaluat <YYYY-MM-DD> (Claude Code automation) ---\nRecomandare: Implementează — <rationale>. Mărime: <S|M|L>. Efort: <low|medium|high|xhigh|max|ultracode>.',
  updated_at = NOW()
...
SET
  status = 'Gata',
  effort = '<low|medium|high|xhigh|max|ultracode — confirm from what the build actually took>',
...
| `TARGET_ITEM_ID` | number | `id`-ul exact al feature-ului de procesat; înlocuiește query-ul în masă din Step 2 |
...
**Sari peste Step 2 (load în masă)** — nu mai rulezi query-ul peste toate feature-urile
`Propus/Planificat/În Focus`. Rulezi aceeași interogare dar **filtrată pe un singur rând**:
...
Dacă rândul nu există sau statusul nu e acționabil → întoarce imediat JSON cu
`outcome="blocked"`, `question="Feature #<id> nu există sau nu mai e acționabil în DB."`.

**Step-urile 3–4 (triage, design, implement, verify) sunt identice** cu flow-ul normal — procesezi
un singur feature, dar cu aceeași calitate de evaluare, același canal de verificare, și aceeași logică
...
preview-ul normal (Step 1 pct. 4) și oprește-l la final — ești în modul standalone.

**Nu printa raportul Step 5** (tabelul de sweep). În loc de raport, **întoarce un JSON structurat
...
```json
{
  ...
  "summary": "<un paragraf: ce s-a construit, fișiere atinse, canalul de verificare, o felie de dovadă>",
